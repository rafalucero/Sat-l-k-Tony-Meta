...
ascii_art = pyfiglet.figlet_format("Marvel Team", font="small")

# Add some color flair
print(f"{Fore.YELLOW}{ascii_art}")
print(f"{Fore.CYAN}Developed by @Sah1bin")
print("")
...
𝗠𝗮𝗿𝘃𝗲𝗹 𝗧𝗲𝗮𝗺 𝗛𝗶𝘁 𝗜̇𝗻𝘀𝘁𝗮𝗴𝗿𝗮𝗺
════════════════
 𝙃𝙞𝙩: {total_hits}
 𝙐𝙨𝙚𝙧𝙣𝙖𝙢𝙚: {username}
 𝙈𝙖𝙞𝙡: {username}@{domain}
 𝙁𝙤𝙡𝙡𝙤𝙬𝙚𝙧𝙨: {followers}
 𝙁𝙤𝙡𝙡𝙤𝙬𝙞𝙣𝙜: {following}
 𝘿𝙖𝙩𝙚: {reg_date}
 𝘽𝙞𝙤: {account_info.get('biography','')}
 𝙍𝙚𝙨𝙩: {rest(username)}
 𝙇𝙞𝙣𝙠: 
